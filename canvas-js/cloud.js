  ...
  //  Draw words in tag cloud
{
  ...
  var  colour;				// Colour scale to fill text
  ...
  colour = d3.scale.category20();	// Use d3 20-colour scale
  ...
  text.enter()				// Terms added to cloud
    ...
    .style( "fill", function( d, i ) {	//  Use d3 colour scale to fill text
      var  hex;			// Hex colour
  ...
  //  Mainline for tag cloud
{
  ...
  //  tooltips when mouse hovers over a term)
  ...
  //  max:  Maximum number of terms per cloud
{
  ...
  for( i = 0; i < tw.length; i++ ) {	// For all tweets
    ...
    for( j = 0; j < tw[ i ].term.length; j++ ) {
      ...
      //  Grab term, strip anything that's not a letter, whitespace,
      //  or underscore (and strip http[s]), ignore empty terms

      term = tw[ i ].term[ j ];
      term = term.replace( /[\W\s]|_/g, "" ).replace( /[\d]/g, "" );
